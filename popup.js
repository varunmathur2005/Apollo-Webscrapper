...
function getAllTables() {
    ...
    const currentPageStatus = document.getElementById('currentPageStatus'); // Get the element
    pageLinks.forEach((pageLink, index) => {
        setTimeout(() => {
            currentPageStatus.textContent = `Scraping page ${index + 1}...`; // Update the page status
                        ...
                        chrome.scripting.executeScript({
                            ...
                            if (chrome.runtime.lastError) {
                                ...
                                if (index === pageLinks.length - 1) {
                                    ...
                                    currentPageStatus.textContent = `All pages scraped!`; // Update when done
                                }
                            }
                        });
                    }
        ...
        }, index * timeBetweenPages); // Wait the specified time between each page to avoid overloading the server
    });
}
